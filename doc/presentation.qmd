---
title: "Development of T cell receptor repertoires during childhood in health and disease"
subtitle: "By Emma A., Bunia A., Othilia FS., Tobias C. & Joanna R."
author: "Group 21"
format:
  revealjs: 
    theme: moon
    embed-resources: true
css: style.css
---

# Introduction {background-image="background_picture.png" background-size="750px" background-position="right"}

------------------------------------------------------------------------

## Data Description

Materials: What data did you use and where did you get it from? Methods: Which modelling did you use? Think of the methods section as a recipe for how to go from raw to results =\> Flow chart? Discuss how data was collected or generated for your study. Include any relevant details or procedures.

------------------------------------------------------------------------

## Data Wrangling {.scrollable}

::: columns
::: {.column width ="40%"} Left column

```{r}
#| class-output: hscroll

library("tidyverse")

read_tsv("./../data/_raw/raw.tsv",
         show_col_types = FALSE) |> 
  head()
```
:::

::: {.column width="60%"}
Right column
:::

::::

------------------------------------------------------------------------

## Antibody Expression

![](./../results/05_plot_1.png){width="500"}

------------------------------------------------------------------------

## Antibody Expression, PCA

![](./../results/06_plot_3.png){.absolute right="300" bottom="10" width="850" height="500"} ![](./../results/06_plot_2.png){.absolute left="600" top="10" width="500" height="300"}

------------------------------------------------------------------------

## HLA Expression

![](./../results/07_plot_1.png){width="900"}

------------------------------------------------------------------------

## HLA Expression, PCA

![](./../results/08_plot_1.png){.absolute width="850" height="500"}

------------------------------------------------------------------------

## Discussion

Discuss the implications of your findings. Interpret the results and relate them to your objectives.

::: incremental
-   Missing data for control makes it difficult to determine significant differences between case and control
-   Results from PCA seems to suggest reducing dimensionality of data by clustering antibodies
-   No conclusive dimensionality reduction shown from PCA on HLA
:::

------------------------------------------------------------------------

## Conclusion

Summarize the main points discussed in the presentation. Emphasize the significance of your work and any future directions.
