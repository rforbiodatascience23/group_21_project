...
read_tsv("./../data/01_dat_load.tsv") |>
  mutate(case_control = case_when(str_detect(sample_tags,"Case") ~ "Case",
    ...
    mutate(id_time=str_c(id,timepoint,sep="_"))|>
    mutate(
      #HLA-A
  ...
  #filter(case_control=="Control",id==17) |>
...
str(str_extract_all("c(test1, test2)", "\\,{0}test\\d?"))
?separate()

str_c(str_extract_all("c(test1, test2)", "\\,{0}test\\d?")[[1]],collapse=",")
