...
read_tsv("./../data/01_dat_load.tsv") |>
  mutate(case_control = case_when(str_detect(sample_tags,"Case") ~ "Case",
    ...
    mutate(HLA_A = str_c(str_extract_all(str_c(str_extract_all(sample_tags, pattern = "HLA\\-{1}A\\*{1}\\d{4}"), sep = ","), "\\d{4}"), sep = ","),
           HLA_B = str_c(str_extract_all(str_c(str_extract_all(sample_tags, pattern = "HLA\\-{1}B\\*{1}\\d{4}"), sep = ","), "\\d{4}"), sep = ","),
           HLA_C = str_c(str_extract_all(str_c(str_extract_all(sample_tags, pattern = "HLA\\-{1}C\\*{1}\\d{4}"), sep = ","), "\\d{4}"), sep = ","),
