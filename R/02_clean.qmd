...
read_tsv("./../data/01_dat_load.tsv") |>
  ...
  mutate(case_control = case_when(str_detect(sample_tags,"Case") ~ "Case",
    ...
    mutate(HLA_A = str_extract(str_extract(sample_tags, pattern = "HLA\\-{1}A\\*{1}\\d{4}"), "\\d{4}"),
           
         HLA_B =str_extract(str_extract(sample_tags, pattern = "HLA\\-{1}B\\*{1}\\d{4}"), "\\d{4}"),
         HLA_C = str_extract(str_extract(sample_tags, pattern = "HLA\\-{1}C\\*{1}\\d{4}"), "\\d{4}"),
