...
patient_data <- patient_data |>
  mutate(
    ...
    id = case_when(
      str_detect(string = sample_tags,
                 pattern = "Subject") ~ as.double(str_extract(string = sample_tags,
                                                              pattern = "(?<=Subject[^,])\\d+")),
      str_detect(string = sample_tags,
                 pattern = "Control") ~ as.double(str_extract(string = sample_tags,
                                                              pattern = "(?<=Control[^,])\\d+")),
      TRUE ~ NA),
    
    timepoint = as.double(str_extract(string = sample_tags,
